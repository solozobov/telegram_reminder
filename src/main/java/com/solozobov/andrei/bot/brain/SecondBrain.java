...
import static com.solozobov.andrei.bot.brain.Dtos.NotificationType.DATE;
import static com.solozobov.andrei.bot.brain.Dtos.NotificationType.INTERVAL;
...
public class SecondBrain extends BaseBrain {
  ...
  private InlineKeyboardMarkup getKeyboard(Notification n) {
    ...
    if (n.type == DATE) {
      buttons.add(list(button("дата", SETUP.getActionKey(new Notification(n.messageId, INTERVAL, n.date, n.time, n.repeated, n.repeatIntervalMinutes)))));
      buttons.add(list(button(n.date.getDayOfWeek() + " " + monthGenitive(n.date) + " " + n.date.getYear(), SET_DATE.getActionKey(n))));
      buttons.add(list(button(n.time.getHour() + ":" + n.time.getMinute(), SET_TIME.getActionKey(n))));
    } else {
      ...
      buttons.add(list(button("через", SETUP.getActionKey(new Notification(n.messageId, DATE, n.date, n.time, n.repeated, n.repeatIntervalMinutes)))));
      buttons.add(list(button(Naming.timeAccusative(minutes), SET_INTERVAL.getActionKey(n))));
    }
    if (n.repeated) {
      buttons.add(list(button("повторять", SETUP.getActionKey(new Notification(n.messageId, n.type, n.date, n.time, false, n.repeatIntervalMinutes)))));
      buttons.add(list(button("каждые " + n.repeatIntervalMinutes + "минут", SET_REPEAT_INTERVAL.getActionKey(n))));
    } else {
      buttons.add(list(button("не повторять", SETUP.getActionKey(new Notification(n.messageId, n.type, n.date, n.time, true, n.repeatIntervalMinutes)))));
    }
  ...
  private final ButtonAction<Notification> SET_DATE = new AuthorizedButtonAction<Notification>("3", NOTIFICATION) {
    protected void perform3(TelegramBot bot, Message message, Notification n) {
      bot.editMessage(message, "Выберите дату", dateSelector(
          n.date,
          getUserTimeZone(),
          newDateToDisplay -> getActionKey(new Notification(n.messageId, n.type, newDateToDisplay, n.time, n.repeated, n.repeatIntervalMinutes)),
          selectedDate -> SETUP.getActionKey(new Notification(n.messageId, n.type, selectedDate, n.time, n.repeated, n.repeatIntervalMinutes))
      ));
    }
  };
  ...
  private final ButtonAction<Notification> SET_TIME = new AuthorizedButtonAction<Notification>("4", NOTIFICATION) {
    protected void perform3(TelegramBot bot, Message message, Notification n) {
      bot.editMessage(message, "Выберите время", timeSelector(
          ...
          getUserTimeZone(),
          selectedTime -> SETUP.getActionKey(new Notification(n.messageId, n.type, n.date, selectedTime, n.repeated, n.repeatIntervalMinutes))
      ));
    }
  };
    ...
    protected void perform3(TelegramBot bot, Message message, Notification n) {
      ...
      bot.editMessage(message, "Выберите время", dayHourMinuteSelector(
          ...
          newMinutesToDisplay -> {
            ...
            return SET_REPEAT_INTERVAL.getActionKey(new Notification(n.messageId, n.type, dateTime.toLocalDate(), dateTime.toLocalTime(), n.repeated, n.repeatIntervalMinutes));
          },
          selectedMinutes -> {
            final ZonedDateTime dateTime = ZonedDateTime.now(timeZone).plusMinutes(selectedMinutes);
            return SETUP.getActionKey(new Notification(n.messageId, n.type, dateTime.toLocalDate(), dateTime.toLocalTime(), n.repeated, n.repeatIntervalMinutes));
          }
      ));
    }
  };
  ...
  private final ButtonAction<Notification> SET_REPEAT_INTERVAL = new AuthorizedButtonAction<Notification>("6", NOTIFICATION) {
    protected void perform3(TelegramBot bot, Message message, Notification n) {
      bot.editMessage(message, "Выберите время", dayHourMinuteSelector(
          ...
          newMinutesToDisplay -> SET_REPEAT_INTERVAL.getActionKey(new Notification(n.messageId, n.type, n.date, n.time, n.repeated, newMinutesToDisplay)),
          selectedMinutes -> SETUP.getActionKey(new Notification(n.messageId, n.type, n.date, n.time, n.repeated, selectedMinutes))
      ));
    }
  };
